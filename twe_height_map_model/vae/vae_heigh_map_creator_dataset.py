from creator.creator_map_dataset_base import CreatorMapDatasetBase
from PIL import Image
import numpy


class VaeHeighMapCreatorDataset(CreatorMapDatasetBase):
	def __init__(self, path, side_size):
		height_map = numpy.array(Image.open(path))

		if len(height_map.shape) > 2:
			if height_map.shape[2] == 4:
				height_map = numpy.delete(height_map, [3], axis=2)
			height_map = numpy.delete(height_map, [1, 2], axis=2)
		else:
			height_map = numpy.reshape(height_map, (height_map.shape[0], height_map.shape[1], 1))

		height_map = numpy.float32(height_map)
		height_map /= 255

		super(VaeHeighMapCreatorDataset, self).__init__(side_size, height_map.shape, 0, False)
		self.height_map = height_map


	def rotate_images_map(self):
		self.height_map = numpy.rot90(self.height_map)


	def get_images_map(self):
		return self.height_map


	def get_full_shape_images_map(self):
		return self.height_map.shape
